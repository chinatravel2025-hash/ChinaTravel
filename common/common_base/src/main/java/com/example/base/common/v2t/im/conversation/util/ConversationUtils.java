...
import static com.example.base.common.v2t.im.constant.IMUserConstant.SYSTEM_EVENT_PUSH;
import static com.example.base.common.v2t.im.constant.IMUserConstant.SYSTEM_EVENT_USER;

import android.text.TextUtils;

import com.example.base.base.SDKConstant;
import com.example.base.common.v2t.im.conversation.bean.ConversationInfo;
import com.example.base.common.v2t.im.conversation.bean.DraftInfo;
...
public class ConversationUtils {
    ...
    public static List<ConversationInfo> convertV2TIMConversationList(List<V2TIMConversation> conversationList) {
        ...
        if (conversationList != null) {
            for (V2TIMConversation conversation : conversationList) {
                ConversationInfo info = convertV2TIMConversation(conversation);
                if(info.getId().equals(SYSTEM_EVENT_USER) || info.getId().equals(SYSTEM_EVENT_PUSH)){
                    continue;
                }
                conversationInfoList.add(info);
            }
        }
    ...
    public static ConversationInfo convertV2TIMConversation(final V2TIMConversation conversation) {
        ...
        if (message == null) {
            long time = DateUtils.getStringToDate("0001-01-01 00:00:00", "yyyy-MM-dd HH:mm:ss");
        ...
        switch (atInfoType) {
            case V2TIMGroupAtInfo.TIM_AT_ME:
                info.setAtInfoText(App.getContext().getString(R.string.ui_at_me));
                info.setAtType(ConversationInfo.AT_TYPE_AT_ME);
                break;
            case V2TIMGroupAtInfo.TIM_AT_ALL:
                info.setAtInfoText(App.getContext().getString(R.string.ui_at_all));
                info.setAtType(ConversationInfo.AT_TYPE_AT_ALL);
                break;
            case V2TIMGroupAtInfo.TIM_AT_ALL_AT_ME:
                info.setAtInfoText(App.getContext().getString(R.string.ui_at_all_me));
        ...
        if(info.getTitle().startsWith("@RBT#")){
            info.setTitle(SDKConstant.OZNAME);
        }
        ...
        info.setType(conversation.getType());
        info.setRecvOpt(conversation.getRecvOpt());
