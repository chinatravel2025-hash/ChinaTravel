package com.example.base.common.v2t.im.conversation.bean;
...
public class ConversationInfo implements Serializable, Comparable<ConversationInfo> {
    ...
    private boolean isXunmi = false;

    public void setXunmi(boolean xunmi) {
        isXunmi = xunmi;
    }

    public boolean isXunmi() {
        return isXunmi;
    }
    ...
    public boolean isDisturb(){
        ...
        if(recvOpt == V2TIMMessage.V2TIM_RECEIVE_NOT_NOTIFY_MESSAGE || V2TIMMessage.V2TIM_NOT_RECEIVE_MESSAGE == recvOpt){
            return true;
        }
    ...
    //客户端自己的数据放这里吧..
    private Object bussinessInfo;

    public void setBussinessInfo(Object bussinessInfo) {
        this.bussinessInfo = bussinessInfo;
    }

    public Object getBussinessInfo() {
    ...
    private boolean isNewMood = false;

    public void setNewMood(boolean newMood) {
        isNewMood = newMood;
    }
    ...
    public void setNewSpace(boolean newSpace) {
        isNewSpace = newSpace;
    }
    ...
    private boolean isMiYou = false;

    public void setMiYou(boolean miYou) {
        isMiYou = miYou;
    }
    ...
    public @NonNull ConversationInfo deepCopy(ConversationInfo info){
        if(info == null)
        ...
        info.setType(type);
        info.setRecvOpt(recvOpt);
        info.setUnRead(this.unRead);
        info.setOrderKey(orderKey);
        info.setLastMessage(lastMessage);
        info.setNewMood(isNewMood);
        info.setNewSpace(isNewSpace);
        info.setConversation(conversation);
        info.setGroup(isGroup);
        info.setTitle(title);
        info.setAtInfoText(atInfoText);
        info.setAtType(atType);
        return info;
    }
