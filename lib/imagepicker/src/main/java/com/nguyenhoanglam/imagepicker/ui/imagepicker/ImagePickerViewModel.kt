...
class ImagePickerViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val showSelectAlbum = MutableLiveData(false)
    val selectedAlbumId = MutableLiveData(ImageHelper.ALL_BUCKET_ID)
    val selectedAlbumName = MutableLiveData("ALL")
        ...
        return withContext(Dispatchers.IO) {
            ...
            contextRef.get()!!.contentResolver.query(
                ...
                while (cursor.moveToNext()) {
                    ...
                    val image = Image(uri, name, mimeType, width, height, size, orientation, data, bucketId, bucketName)
                    images.add(image)
                }
                cursor.close()
                images
