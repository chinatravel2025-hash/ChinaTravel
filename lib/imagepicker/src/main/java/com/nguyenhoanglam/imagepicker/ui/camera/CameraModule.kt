        ...
        try {
            if (DeviceHelper.isMinSdk29) {
                    ...
                    if (newFileUri != null) {
                        ...
                        val images = arrayListOf(
                            Image(newFileUri!!,
                                currentFileName!!,
                                "image/jpeg",
                                orientedBitmap.width, orientedBitmap.height, orientedBitmap.byteCount.toLong(),
                                0, PathUtils.getPath(context, newFileUri!!) ?: "", 0, config.subDirectory!!)
                        )
                ...
                MediaScannerConnection.scanFile(
                    ...
                    val bitmapOptions = ImageHelper.decodeImageBounds(currentFilePath!!)
                    val images = arrayListOf(
                        Image(currentFileUri!!, currentFileName!!,
                            "image/jpeg",
                            bitmapOptions.outWidth, bitmapOptions.outHeight, File(currentFilePath!!).length(),
                            0, currentFilePath!!, 0, config.subDirectory!!)
                    )
                    imageReadyListener.onImageReady(images)
                    reset(context)
                }
