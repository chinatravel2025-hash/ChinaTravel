...
public final class PermissionInterceptor implements OnPermissionInterceptor {
   ...
   private PopupWindow mPermissionPopup;
    ...
    public PermissionInterceptor(@Nullable String permissionTitle ,@Nullable String permissionDescription) {
    ...
    public void launchPermissionRequest(@NonNull Activity activity, @NonNull List<String> allPermissions, @Nullable OnPermissionCallback callback) {
        ...
        if (showPopupWindow) {
            ...
            HANDLER.postDelayed(() -> {
                ...
                showPopupWindow(activity, decorView,mPermissionTitle, mPermissionDescription);
            }, 300);
        } else {
    ...
    public void deniedPermissionRequest(@NonNull Activity activity, @NonNull List<String> allPermissions,
        ...
        if (deniedPermissions.size() == 1) {
            ...
            if (Permission.ACCESS_BACKGROUND_LOCATION.equals(deniedPermission)) {
                Toast.makeText(activity,activity.getString(R.string.common_permission_background_location_fail_hint, backgroundPermissionOptionLabel),Toast.LENGTH_SHORT).show();
                return;
            }

            if (Permission.BODY_SENSORS_BACKGROUND.equals(deniedPermission)) {
                Toast.makeText(activity,activity.getString(R.string.common_permission_background_sensors_fail_hint, backgroundPermissionOptionLabel),Toast.LENGTH_SHORT).show();
                return;
            }
        }
    ...
    private void showPopupWindow(Activity activity, ViewGroup decorView, String title, String message) {
        ...
        TextView tvTitle = mPermissionPopup.getContentView().findViewById(R.id.tv_title);
